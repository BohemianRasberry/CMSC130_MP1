...
class Quine:
    ...
    def toMultiply(self, element1, element2):
        '''
        Multiply 2 minterms
        Args:
        ...
        result = []
        for i in element1:
            if i + "'" in element2 or (len(i) == 2 and i[0] in element2):
                return []
            else:
                result.append(i)
        for i in element2:
    ...
    def multiplyEXP(self, exp1, exp2):
        '''
        ...
        for i in exp1:
            for j in exp2:
                temp = self.toMultiply(i, j)
                result.append(temp) if len(temp) != 0 else None
        return result

    def toRefine(self, my_list, dc_list):
        '''
    ...
    def identify_newVAR(self, listPI, variables):
        '''
    ...
    def findMinterms(self, elementsPI):
        '''
    ...
    def deleteTerms(self, _chart, termsBinary):
        '''
    ...
    def toReplace(self, temp, groups, m, marked, should_stop):
        '''
    ...
    def main(self):
        ...
        if choice[0].lower() == 'y':
            ...
            if len(chart) == 0:
                ...
                P = [[self.identify_newVAR(j, variables) for j in chart[i]] for i in chart]
                while len(P) > 1:  # Keep multiplying until we get the SOP form of P
                    P[1] = self.multiplyEXP(P[0], P[1])
                    P.pop(0)
                # Choosing the term with minimum variables from P
                final_result = [min(P[0], key=len)]
                final_result.extend(self.identify_newVAR(i, variables)
            ...
            if len(chart) == 0:  # If no minterms remain after removing EPI related columns
                final_result = [self.identifyVAR(i)
                                for i in EPI]  # Final result with only EPIs
            else:  # Else follow Petrick's method for further simplification
                P = [[self.identifyVAR(j) for j in chart[i]] for i in chart]
                while len(P) > 1:  # Keep multiplying until we get the SOP form of P
                    P[1] = self.multiplyEXP(P[0], P[1])
                    P.pop(0)
                # Choosing the term with minimum variables from P
                final_result = [min(P[0], key=len)]
                final_result.extend(self.identifyVAR(i)
                                    for i in EPI)  # Adding the EPIs to final solution
            print('\n\nSolution: F = ' + ' + '.join(''.join(i)
                                                    for i in final_result))
