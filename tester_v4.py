...
def main():
    mt = [int(i) for i in input("Enter the minterms: ").strip().split()] #List of minterms
    dc = [int(i) for i in input("Enter the don't cares(If any): ").strip().split()] #List of maxterms
    mt.sort() #Arrange mt list in ascending order
    minterms = mt+dc
    minterms.sort() #Arrange minterms in ascending order
    size = len(bin(minterms[-1]))-2 #Returns the length of the largest binary number
    groups,all_pi = {},set() #possible to utilize sort/set more

    # Primary grouping starts
    for minterm in minterms:
        try:
            grp1 = groups[bin(minterm).count('1')].append(bin(minterm)[2:].zfill(size))
            print('line 102')
            print(grp1)
        except KeyError:
            grp2 = groups[bin(minterm).count('1')] = [bin(minterm)[2:].zfill(size)]
            print('line 106')
            print(grp2)
    ...
    for i in sorted(groups.keys()):
        print("%5d:"%i) # Prints group number with space
        for j in groups[i]:
    ...
    while True:
        tmp = groups.copy() #Emptied groups with binary and makes temp
